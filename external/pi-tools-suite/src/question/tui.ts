...
export function createQuestionnaireComponent(
	...
	tui: QuestionTui,
	theme: QuestionTheme,
	done: (value: QuestionSelection[] | null) => void,
): QuestionComponent {
		...
		function renderSelectableLine(add: AddLine, selected: boolean, text: string, zone: Omit<ClickZone, "row" | "startColumn" | "endColumn">, width: number, foreground = "text"): void {
			...
			const row = add(selected
				? paintLine(theme, line, width, { foreground: "text", background: "selectedBg", bold: true })
		...
		function renderHeader(add: AddLine, title: string, width: number): void {
			add(paintLine(theme, formatHeader(title, width), width, { foreground: "accent", bold: true }));
		}
		...
		function renderTabs(add: AddLine, width: number): void {
			...
			for (let index = 0; index < questions.length; index += 1) {
				...
				styledParts.push(active === index
					? styleText(theme, label, { foreground: "text", background: "selectedBg", bold: true })
			...
			styledParts.push(active === "review"
				? styleText(theme, reviewLabel, { foreground: "text", background: "selectedBg", bold: true })
...
export async function runQuestionnaire(
	...
	return ctx.ui.custom<QuestionSelection[] | null>((tui, theme, _keybindings, done) => (
		createQuestionnaireComponent(questions, ctx, tui, theme, done)
	));
}
