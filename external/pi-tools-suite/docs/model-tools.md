...
```bash
pi install npm:pi-model-tools
```

For project-local installation, use Pi's local settings flag:

```bash
pi install -l npm:pi-model-tools
```

## Behavior

- Claude models get Claude Code style aliases: `Read`, `Edit`, `Write`, `Bash`, `Grep`, `Glob`, `LS`.
- GPT/Codex models get the `shell` alias, plus `apply_patch`.
- Other models, including GLM/Z.ai, fall back to Claude-style aliases unless detected as GPT/Codex.
