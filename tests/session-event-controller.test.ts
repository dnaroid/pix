...
describe("AppSessionEventController", () => {
	...
	it("offers a manual retry toast for a retryable streaming error when auto-retry is disabled", () => {
		...
		buildController(false).handleSessionEvent({
			type: "message_update",
			assistantMessageEvent: { type: "error", reason: "error", error: { errorMessage: rateLimit } },
		} as unknown as AgentSessionEvent);
		...
		buildController(false).handleSessionEvent({
			type: "message_update",
			assistantMessageEvent: { type: "error", reason: "aborted", error: { errorMessage: "Operation aborted" } },
		} as unknown as AgentSessionEvent);
		assert.deepEqual(toasts, []);
		assert.deepEqual(toastActions, []);
		...
		buildController(false).handleSessionEvent({
			type: "message_update",
			assistantMessageEvent: { type: "error", reason: "error", error: { errorMessage: "insufficient_quota: billing quota exceeded" } },
		} as unknown as AgentSessionEvent);
		assert.deepEqual(toasts, []);
		assert.deepEqual(toastActions, []);
	});
