...
<section class="grid min-h-0 grid-rows-[auto_minmax(0,1fr)]" aria-label="Current session activity">
  <div class="border-b border-sidebar-border p-2.5">
    <div class="flex items-center justify-between gap-2">
      <div class="flex min-w-0 items-center gap-1.5">
        ...
        <h2 class="text-xs font-semibold text-foreground">Todos</h2>
      </div>
      {#if hasOpenSessionTodos(todoSnapshot)}
        <span class="text-[10px] text-muted-foreground">{openTodoCount} open · {todoCounts.completed} done</span>
      {/if}
    </div>
