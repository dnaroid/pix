...
<section aria-label={isStaged ? "Staged changes" : "Working tree changes"}>
  <header class="sticky top-0 z-[1] flex h-8 items-center gap-1.5 border-y border-sidebar-border bg-chrome px-2">
    <strong class="text-[11px] font-medium">{isStaged ? "Staged" : "Changes"}</strong>
    <span class="font-mono text-[11px] text-muted-foreground">{query ? `${visible.length}/` : ""}{changes.length}</span>
    {#if changes.length}
      ...
      <button class="inline-flex h-6 items-center gap-1 rounded-sm px-1 text-[11px] text-muted-foreground hover:bg-panel-hover hover:text-foreground focus-visible:outline-2 focus-visible:outline-ring disabled:opacity-40" type="button" disabled={busy}
  ...
  {#each visible as change (change.path)}
    ...
    <div class="group flex h-8 min-w-0 items-center gap-1 px-2 hover:bg-sidebar-accent focus-within:bg-sidebar-accent">
      <button class="flex h-8 min-w-0 flex-1 items-center gap-1.5 text-left focus-visible:outline-2 focus-visible:-outline-offset-2 focus-visible:outline-ring" type="button" title={`${change.path} · ${gitChangeLabel(change, scope)}`} disabled={busy} onclick={() => onOpenDiff(change.path, scope)}>
        <span class={["w-3 shrink-0 text-center font-mono text-[11px] font-bold", tone(change)]}>{gitChangeCode(change, scope)}</span>
        <span class="truncate font-mono text-[11px]">{change.path.slice(separator + 1)}</span>
        {#if separator >= 0}<span class="min-w-0 truncate text-[10px] text-muted-foreground">{change.path.slice(0, separator)}</span>{/if}
      </button>
      {#if stats.additions !== undefined || stats.deletions !== undefined}
        <span class="flex shrink-0 gap-1 font-mono text-[10px] tabular-nums" aria-label={`${stats.additions ?? 0} additions, ${stats.deletions ?? 0} deletions`}>
  ...
  {:else}
    <p class="px-3 py-2 text-[11px] text-muted-foreground">{query && changes.length ? "No matching files." : isStaged ? "Choose the files to include in your commit." : "No unstaged changes."}</p>
  {/each}
</section>
