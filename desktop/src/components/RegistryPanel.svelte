      ...
      <div class="rounded-md border border-sidebar-border bg-panel p-1.5">
        ...
        >
          ...
          <span class="min-w-0 flex-1">
            ...
            <span class="block truncate text-xs text-muted-foreground">{projectConflictCount > 0 ? `${projectConflictCount} ${projectConflictCount === 1 ? "item needs" : "items need"} review` : projectPendingItems.length > 0 ? `${projectPendingItems.length} ${projectPendingItems.length === 1 ? "change" : "changes"} to sync` : "Tasks, plans and TODO are up to date"}</span>
          </span>
