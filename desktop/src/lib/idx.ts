...
/** Explicit task paths only; never turn a blank audit into a repository-wide scan. */
export function idxAuditPaths(input: string): string[] {
  return [...new Set(input.split(/[,\r\n]+/u).map((part) => part.trim()).filter(Boolean))];
}

export function idxValidAuditPaths(paths: readonly string[]): boolean {
