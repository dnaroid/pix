...
- One-time migration from the legacy strict-JSON `.pi/tasks.json` file.
...
1. The sidebar starts on `Tasks`, remembers its width/collapsed state locally,
   and keeps a compact tab rail available while collapsed.
...
7. If `.pi/tasks.jsonc` is absent and legacy `.pi/tasks.json` exists, Pix reads
   and validates the legacy document, adds the schema reference when absent,
   writes `.pi/tasks.jsonc`, deletes the legacy file, and returns the migrated
   document. If JSONC already exists, it wins and the legacy file is ignored.
...
- Missing storage means no tasks; malformed or empty JSON/JSONC is an error.
- A legacy task file is migrated at most once because successful migration
  removes `.pi/tasks.json`.
...
- Rust tests in `desktop/src-tauri/src/lib.rs` cover missing/read/write/malformed
  JSONC, validation, workspace confinement, and legacy JSON migration.
...
- Confirmed by code: `desktop/src-tauri/src/lib.rs` owns JSONC persistence,
  validation, confinement, atomic replacement, and legacy migration.
