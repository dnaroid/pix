...
- ACP connection state is not shown as persistent status-bar chrome. Connection failures continue through the existing error feedback/reconnect path. When ACP is ready and its active-session-scoped prompt is running, the underlying conversation's composer border uses the semantic primary color with a restrained pulse (static primary with reduced motion) instead of a separate working indicator. Auxiliary workbench tabs retain that underlying runtime state; background conversation activity remains represented only by its existing tab status icon. The transcript does not duplicate this state with a bottom `working` line.
...
- Switching away from a materializing draft invalidates that generation and restores the snapshotted draft text and attachments before composer ownership changes, so the draft remains available for return. If its workspace/client/draft ownership becomes stale, any created ACP session is discarded asynchronously and a late completion cannot replace the selected session or send the original prompt there, nor surface an obsolete error.
...
- `desktop/src/app/prompt-submit.test.ts` verifies that the first draft user message renders and clears the composer before materialization resolves, without duplicate prompt submission; `desktop/src/app/draft-tab-selection.test.ts` verifies restoration before switching away from a materializing draft.
