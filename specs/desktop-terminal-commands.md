...
- A non-empty composer input beginning with `!!` is interactive/raw-terminal mode, matching the Pix TUI contract that `!!command` uses the raw terminal. Desktop opens the Package Scripts / Terminal sidebar, starts a shell PTY in the workspace, writes the command followed by Enter, focuses the terminal, and leaves that terminal session open for further interactive input.
...
- `!!` reuses the existing Desktop PTY stack (`package_terminal_start_shell` + `package_terminal_write` + `TerminalView`) rather than introducing a second terminal implementation.
...
- Desktop submit tests verify that `!!` opens the interactive terminal without materializing/queueing a Pi session and that `!` still uses the one-shot ACP bash path.
