...
- Desktop launches its ACP backend with `PIX_CONFIG_PROFILE=desktop`. ACP user/project reads therefore resolve to `pix-desktop.jsonc` / `.pi/pix-desktop.jsonc`; the default profile remains `pix.jsonc` / `.pi/pix.jsonc` for TUI-compatible ACP use.
