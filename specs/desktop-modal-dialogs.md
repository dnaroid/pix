...
- Model + Thinking focuses its model-search combobox. Model rows are listbox options outside the normal Tab sequence; its existing arrow/search/thinking controls and staged Apply semantics remain unchanged. In select mode, `Enter` confirms the staged model/thinking selection from the search field, a model row, or a thinking chip exactly like the `Apply` button (including its dirty/disabled guards); footer buttons such as `Set default`/`Cancel` keep their own native `Enter` activation. In visibility-manage mode, `Enter` keeps toggling the visibility of the highlighted model instead of applying.
