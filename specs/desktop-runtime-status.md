...
- Context danger thresholds match the TUI on the numeric percentage only: up to 30% uses success, 31–50% warning, and above 50% error semantics. The compact Context scale itself does not inherit those semantic colors: occupied capacity is rendered as a neutral light gray, with free capacity visually separate.
- When a prepared DCP context map is available, the compact status-bar Context scale keeps its occupied segments neutral light gray rather than turning the persistent meter into a multi-color bar. Hovering or keyboard-focusing the context control shows absolute context tokens/current window plus the categorical color legend for other/retained content, compression candidates, protected incomplete tool groups, and current summaries with approximate category token volumes. The live DCP `tokensSaved` estimate is also shown directly in the status bar when available. The saved value is the extension's running estimate of context removed by pruning/compression, not the durable statistics report's measured commit gain and not a billing counter.
...
- Model-limit color thresholds match the TUI on the numeric percentage only: below 20% remaining is error, below 50% remaining is warning, otherwise success. The compact Usage scale uses the same neutral light-gray fill as Context; percentage text keeps its semantic success/warning/error color. The TUI long-window projected-exhaustion warning heuristic is retained, but its icon does not add another threshold color.
