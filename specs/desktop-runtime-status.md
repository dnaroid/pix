...
- Once an active Desktop session runtime is ready, the status bar shows the current context fill percentage and a slightly extended horizontal capacity track.
- Context danger thresholds match the TUI on the numeric percentage only: up to 30% is the normal/success range, 31–50% warning, and above 50% error. Desktop renders the normal numeric percentage in neutral light gray instead of success green; warning/error keep their semantic colors. The progress track keeps its existing capacity/DCP category coloring.
- When a prepared DCP context map is available, the occupied portion of the status-bar track remains categorically colored for other/retained content, compression candidates, protected incomplete tool groups, and current summaries; free capacity remains visually separate. Hovering or keyboard-focusing the context control shows absolute context tokens/current window plus a compact color legend with approximate category token volumes. The live DCP `tokensSaved` estimate is also shown directly in the status bar when available. The saved value is the extension's running estimate of context removed by pruning/compression, not the durable statistics report's measured commit gain and not a billing counter.
...
- When the active model/provider exposes account quota data, the status bar shows hourly and/or weekly remaining percentages with compact horizontal tracks and reset countdowns. Weekly tracks are divided into seven equal day slices; their hover text labels those slices from the reported window timing. The provider still supplies one aggregate weekly remaining percentage, so the UI does not invent per-day usage values.
- Model-limit color thresholds match the TUI on the numeric percentage only: below 20% remaining is error, below 50% remaining is warning, otherwise the normal/success range. Desktop renders that normal numeric percentage in neutral light gray instead of success green; warning/error keep their semantic colors. Quota progress fills remain neutral gray. The TUI long-window projected-exhaustion heuristic is retained and its triangle uses the semantic warning color so the projection is visually recognizable even when the numeric percentage is still in the normal range.
- Clicking `Usage` opens the on-demand session-spend popover defined in [session-usage.md](./session-usage.md); it does not force a provider quota refresh. The popover is a compact `provider → model → tokens · cost` breakdown. Parent-agent, async-subagent, retry, and fallback calls are merged into the model that actually incurred them. Model names use the normal Pix model colors. The popover does not show account quota, provider/session-share percentages, or a separate agent/subagent subtotal.
- Normal model-quota refresh still happens when the active runtime/model changes and every five minutes. The `Usage` control remains available while a prompt or quota refresh is running because reading recorded spend is independent of provider quota I/O.
...
- While Desktop is starting, a session tab is switching through history/runtime activation, or the first runtime-status snapshot has not arrived, the status bar keeps its normal Model/Context/workspace/Usage footprint with non-interactive skeleton placeholders for values that are not known yet. Values that are already available remain real; connection error/stopped states are never hidden behind loading placeholders.
- A UI-only New Conversation draft has no runtime context/DCP state, so its status bar keeps a non-interactive Context skeleton in that slot instead of inventing telemetry. The real workspace project/branch identity remains visible, the Model slot becomes real as soon as draft configuration is available, and quota windows replace the Usage skeleton when the staged model/thinking provider exposes usage data.
- Draft quota is loaded through the existing sessionless draft-config path and never allocates an ACP/Pi session. Changing the staged model or thinking level invalidates the previous draft quota and starts a best-effort refresh for the new route. Opening Usage on a draft does not create a session or force another quota request.
...
- DCP and session-usage requests have independent generation guards within a live per-session owner. Forget removes that owner rather than retaining a closed-ID tombstone; reopening creates a distinct owner, so repeated local counters cannot let an old completion update the new runtime. A reset lifecycle generation invalidates status, DCP, and usage responses plus their busy-state cleanup, even when the same session and client are reused. Stale responses cannot recreate missing state.
...
- The same status-bar chrome also hosts the separate Session activity HUD only while there is hidden live activity. With the inspector closed, each of the first three active agent icons is its own compact control; hovering or keyboard-focusing an icon shows the same key live details as the inspector (agent/run identity, status, elapsed time, task/scope, model, latest activity, and retry count), while clicking it opens the inspector. Additional active agents collapse into a compact `+N` control that opens the inspector. Completed/total Plan progress is a neighboring control whose hover/focus surface shows the current open item, prioritizing `in_progress`, then `pending`, then `deferred`, together with its active form/description and available thinking, owner, and blocker metadata. Clicking Plan progress also opens the inspector. Empty sessions and an already-open inspector render no Session status control. Session Plan/Subagent state comes from pushed session-state snapshots rather than the runtime-status request, so opening or updating that HUD does not add quota/context polling.
- The Session inspector no longer duplicates DCP telemetry; it contains the Agents and Plan activity sections only. Their independently collapsible native `<details>` accordions preserve the existing first-snapshot/manual-toggle behavior.
- The compact status-bar context track is the always-visible DCP capacity visualization. Total occupied/free capacity comes only from the already-held live SDK `tokens` and positive `contextWindow` denominator; the separately reported percentage is not a denominator. Invalid or unknown context produces neutral unknown capacity rather than invented free space, and over-capacity samples fill the bounded track while retaining their actual textual percentage.
...
- The status-bar map fits those estimates to SDK occupancy: when their total exceeds observed occupied tokens all estimates scale down uniformly; otherwise the unclassified occupied remainder (including system/tool overhead) joins retained/other. Free capacity still comes from SDK usage, not the DCP estimate. Small category shares remain represented within split cells. A missing/invalid DCP snapshot leaves occupied capacity as neutral gray rather than falsely reporting zero candidates. Missing SDK capacity prevents drawing token proportions even when DCP estimates are available.
- The hover legend follows the same left-to-right category order as the status-bar track. Durable journal/block/gain details remain available independently through the existing on-demand Context/DCP statistics popover.
...
- Session spend uses the separate private `pix/session/usage` request. ACP gets the complete in-memory session tree through Pi's async `getTree()` RPC and aggregates all persisted billed entries, including abandoned branches, because provider billing is not undone by a fork/rewind. The request never performs provider quota I/O.
...
- `desktop/src/app/status-bar-skeleton.ts`
- `desktop/src/app/desktop-status-bar-view-model.svelte.ts`
- `desktop/src/app/desktop-status-bar-view-model.test.ts`
...
- Desktop ACP-client tests cover independent runtime-status, DCP, and session-usage request shapes plus response validation for context, DCP text, quota windows, and provider billing totals.
- Desktop/ACP draft-config tests cover selected-model/thinking quota requests without a session id. Status-bar policy tests cover startup, draft, history/runtime tab-switch activation, first-snapshot readiness, and connection-error precedence; source-level visual regression coverage pins the non-interactive Model/Context/workspace/Usage skeleton slots while keeping real project/branch and draft quota visible as they become available.
...
- Desktop visual regression tests cover the status-bar DCP category legend, inline savings, neutral progress-track policy, seven-slice weekly quota track, and the absence of the duplicate DCP Session-inspector section. They also cover the remaining native Session inspector accordions, their summary metrics, first-snapshot defaults, the compact closed-inspector agent/Plan status summary, and the absence of reactive `open` bindings that could reset accordion choices on updates. Activity-policy tests cover the close-only pane rule: activity never auto-opens the inspector, while an open inspector closes once both visible Plan and Agents activity are empty.
- Runtime-status lifecycle tests exercise deferred completions across forget/reopen and reset, including stale DCP/status/session-usage responses and busy-state cleanup.
