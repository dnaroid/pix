...
- DCP and session-usage requests have independent per-session generation guards so a late response from a forgotten/reloaded runtime cannot overwrite the current session snapshot. Runtime-status generations retain a monotonic tombstone on forget; a reset lifecycle generation invalidates status, DCP, and usage responses plus their busy-state cleanup, even when the same session and client are reused. Stale responses cannot recreate missing state.
