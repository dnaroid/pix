...
- Desktop visual regression tests cover the status-bar DCP category legend, inline savings, neutral progress-track policy, seven-slice weekly quota track, and the absence of the duplicate DCP Session-inspector section. They also cover the remaining native Session inspector accordions, their summary metrics, first-snapshot defaults, and the absence of reactive `open` bindings that could reset user choices on updates. Activity-policy tests cover first-display reconciliation and late activity snapshots.
