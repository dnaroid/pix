...
- `/model` and `/thinking` without an argument expose the same combined `Select model & thinking` popup. The model and thinking status-line targets open that same popup. When first-prompt model routing is enabled, that popup also contains `Auto`.
- On a real session the popup starts from `ModelRuntime.getAvailableSnapshot()` even when the session has an explicit `enabledModels` scope. On a UI-only draft Pix builds the same locally available model catalogue, including providers registered by extensions, without allocating an `AgentSession`; the temporary extension runtime used for provider discovery is invalidated immediately afterward. Both paths then apply the TUI `visibleModels` whitelist from `pix.jsonc`; Desktop maintains its own whitelist in `pix-desktop.jsonc`.
...
- When model routing is enabled, `Auto` is pinned above the fuzzy-ranked concrete model list regardless of the current model or query.
...
- `Enter` applies the staged model and thinking together. On a real session a concrete model mutates that session; selecting `Auto` instead opens or reuses a UI-only New Conversation draft and stages routing there, leaving the live session unchanged. On an existing UI-only draft the selection updates only draft-local state and becomes the model/thinking override, or Auto routing decision, supplied when the first normal prompt materializes the runtime. `Escape` closes the popup and discards the staged values.
- `Set default` in the picker (click or `Ctrl+D`) persists the currently staged concrete model + thinking pair as the future-session default without applying it to the current session. If `Auto` is staged, the action enables routing and persists `modelRouting.default: true` instead. Choosing a concrete default clears that Auto-default flag.
- After a successful `Enter`, Pix stores the effective selected model/thinking pair in user `~/.config/pi/pix.jsonc` `thinkingByModel`. Escape/cancel never persists staged-only changes. This TUI preference is independent from Desktop's `pix-desktop.jsonc` preference and project config cannot override it; see `model-thinking-preferences.md`.
...
- A draft staged in unresolved `Auto` shows only `Auto` in the status line. The thinking glyph/value is omitted until routing has actually selected a tier.
