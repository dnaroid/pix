...
- Porting unrelated terminal pickers, settings, authentication, tree navigation,
  share, or process-lifecycle commands without matching Desktop UI or Pi RPC support.
...
- Exact `/new` starts a fresh Desktop conversation and `/new_tab` opens one in a
  new tab. `/resume [path]`, `/import [path]`, `/search [query]`,
  `/delete [query]`, `/jump [query]`, `/history [query]`, `/enhance [draft]`,
...
- On a UI-only New Conversation draft, argument-free `/model` and `/thinking`, their explicit argument forms, the status-bar control, and the Command Palette model/thinking action use the draft config catalogue instead of requiring an ACP session id. Applying a choice updates draft-local config only; the selected model/thinking values are forwarded as a one-session override when the first normal prompt lazily creates the session.
...
- `/tree`, `/settings`, `/share`, `/trust`, `/login`, `/logout`, and `/bug` still require additional APIs or Desktop UI.
