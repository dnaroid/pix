...
- DCP, Agents, and Plan are independent native keyboard-accessible accordions with informative summary headers. DCP starts collapsed; Agents and Plan start collapsed without a snapshot, then use their first available snapshot to expand only when live agents or an open plan exist. A manual toggle before that snapshot takes precedence, and ordinary later snapshot updates never reset the accordion. Session switches reset these section defaults for the newly displayed session. The DCP capacity map and its metric semantics are defined in [desktop-runtime-status.md](./desktop-runtime-status.md).
