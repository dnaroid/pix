...
- The status HUD is intentionally denser than the inspector. While the inspector is closed, each of the first three live Subagents contributes its configured agent icon (falling back to the generic agent icon) as an individual accessible control, with excess icons compacted behind a `+N` control. Hovering or keyboard-focusing one of those icons reveals the same key live details as the Agents inspector row: run/agent identity, status, elapsed time, task or scope, model, latest activity, and retry count. An open Plan contributes a neighboring `completed/total` progress control such as `2/5`; hovering or focusing it reveals the current open item, preferring `in_progress`, then `pending`, then `deferred`, including active form/description plus thinking, owner, and blockers when present. Clicking any visible HUD control opens the inspector. Once opened, the summary disappears completely; the inspector's own close control and the existing Session Activity command own closing.
