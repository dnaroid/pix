...
export function createLazySessionManager(cwd: string, options: LazySessionManagerOptions = {}): SessionManager {
  const sessionDir = options.sessionDir ?? getDefaultSessionDir(cwd, options.agentDir);
  return SessionManager.create(cwd, sessionDir);
}

export function continueRecentLazySessionManager(cwd: string, options: LazySessionManagerOptions = {}): SessionManager {
  const sessionDir = options.sessionDir ?? getDefaultSessionDir(cwd, options.agentDir);
  const mostRecent = findMostRecentSession(sessionDir, options.sessionDir ? cwd : undefined);
  return mostRecent
    ? openLazySessionManager(mostRecent, { ...options, cwdOverride: cwd, sessionDir })
...
class LazySessionManager {
  ...
  usesDefaultSessionDir(): boolean {
    return this.hydrated?.usesDefaultSessionDir() ?? this.sessionDirPath === getDefaultSessionDirPath(this.cwdPath);
  }
  ...
  getBranch(fromId?: string): SessionEntry[] {
    ...
    if (!this.byId.has(fromId)) return this.hydrate().getBranch(fromId);
    return [...this.entries];
  }
...
function getDefaultSessionDirPath(cwd: string, agentDir = getAgentDir()): string {
...
function getDefaultSessionDir(cwd: string, agentDir = getAgentDir()): string {
...
function findMostRecentSession(sessionDir: string, cwd?: string): string | null {
  ...
  try {
    const files = readdirSync(resolvedSessionDir)
      ...
      .map((path) => ({ path, header: readSessionHeaderFast(path) }))
      .filter((file) => file.header !== undefined && (!resolvedCwd || resolve(file.header.cwd) === resolvedCwd))
